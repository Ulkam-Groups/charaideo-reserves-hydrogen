...
import {findChapterCollection, selectStockedChapterProduct} from '../app/lib/chapter-inventory.ts';

test('five zero-inventory products keep Chapter I sealed', () => {
  assert.equal(selectStockedChapterProduct(Array.from({length: 5}, () => ({totalInventory: 0}))), null);
});

test('adding stock to any product opens its collection', () => {
  const stocked = {totalInventory: 12, handle: 'chapter-one-tea'};
  assert.equal(
    selectStockedChapterProduct([...Array.from({length: 4}, () => ({totalInventory: 0, handle: 'sealed'})), stocked]),
    stocked,
  );
});
