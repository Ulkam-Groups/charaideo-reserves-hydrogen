...
import {chapterState, partitionReserveCollections, type ReserveCollection} from '../app/lib/reserve-list.ts';
...
test('chapters are sorted numerically and separated from other collections', () => {
  ...
  assert.deepEqual(result.chapters.map(({title}) => title), ['Chapter I', 'Chapter II', 'Chapter III']);
  assert.deepEqual(result.others.map(({title}) => title), ['Craft Collection', 'Reserve Collection']);
});
